...
SKIN.Colours.Tree = {}
SKIN.Colours.Tree.Lines = GWEN.TextureColor(4 + 8 * 10, 508) ---- not not not
...
function SKIN:PaintTextEntry(panel, w, h)
	...
	if
		(panel.GetPlaceholderText and panel.GetPlaceholderColor and panel:GetPlaceholderText() and
			panel:GetPlaceholderText():Trim() ~= "" and
			panel:GetPlaceholderColor() and
			(not panel:GetText() or panel:GetText() == ""))
	 then
...
function SKIN:PaintCollapsibleCategory(panel, w, h)
	if (h <= panel:GetHeaderHeight()) then
		...
		if (not panel:GetExpanded()) then
			self.tex.Input.ComboBox.Button.Down(w - 18, h / 2 - 8, 15, 15)
		end
		return
	end
